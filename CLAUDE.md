...
Required (`envconfig`, fatal if missing): `MQTT_BROKER_URL`, `MQTT_CLIENT_ID`, `MQTT_USERNAME`, `MQTT_PASSWORD`, `HOSTNAME_OR_IP`, `DAHUA_USERNAME`, `DAHUA_PASSWORD`. Optional: `MQTT_TOPIC` (default `doorbell/pressed`), `MQTT_AVAILABILITY_TOPIC` (default `doorbell/availability`), `MQTT_DISCOVERY_PREFIX` (empty = HA discovery disabled; `homeassistant` matches HA's default), `HEALTH_PORT` (default `8080`), `APP_ENV=development` for human-readable console logging instead of JSON.
...
- **`pkg/dahua`** — opens a long-poll HTTP GET to `/cgi-bin/eventManager.cgi?action=attach&codes=[AlarmLocal]&heartbeat=5` using digest auth (`icholy/digest` transport), built with `http.NewRequestWithContext` so cancelling the context aborts the stream immediately. Every connection runs under an **idle watchdog** (`streamIdleTimeout` = 3 missed heartbeats = 15s): any bytes from the camera reset it (raw reads, not parsed lines — heartbeat framing doesn't matter), and silence tears the stream down for reconnect, since OS TCP keepalives would otherwise leave a dead stream — and `IsConnected()==true` — undetected for minutes. The watchdog is armed before `Do`, so it also bounds connect + digest handshake. Scans the response line-by-line; `isDoorbellPressed` parses `Code=...;action=...;` lines and only reports `AlarmLocal` with `action=Start` (or absent action). `onEvent` is called with no payload — the event *is* the signal. `Listen` wraps `listen` in `backoff/v5` retry with `MaxElapsedTime(0)` (retry until cancelled) and `MaxInterval` lowered to 10s; the backoff **resets once a connection is established**, so only consecutive failures grow the reconnect delay. `Probe` is the read-only authenticated camera check (`magicBox.cgi?action=getMachineName`) that health uses; the shared `http.Client` intentionally has no client-wide timeout (the stream is endless), so bounded calls pass a context deadline.
- **`pkg/mqtt`** — `New()` connects with paho `ConnectRetry` enabled (a down broker at boot no longer kills the process), the reconnect ceiling lowered to 30s, and keepalive lowered to 10s (+5s ping timeout) so a half-open broker socket stops reporting `IsConnectionOpen` within seconds, not paho's default ~40s. `Publish` is a non-blocking send onto a buffered channel of 100 (full queue = drop the new event). `ProcessQueue` delivers; while the broker is unreachable it holds the current event and polls once a second, and any event older than 30s (`maxEventAge`) is **dropped, not delivered late** — stale doorbell events are worthless by design. QoS 1 (the PUBACK turns a dead-socket publish into a logged drop instead of silence) bounded by `publishTimeout` = 5s, no app-level retry on failure (no duplicate presses), not retained. The success log carries `latency_ms` (queue-entry → broker ack). **Availability** is retained `online`/`offline` on `MQTT_AVAILABILITY_TOPIC`: a Last Will covers process death, `OnConnect` re-announces the stored state after every reconnect, `Disconnect` leaves an explicit `offline` (a clean disconnect suppresses the will), and `main.go` wires `dahua.Client.OnConnectionChange` to `PublishAvailability` so the topic mirrors the event stream — availability publishes are serialized under a mutex so the retained value converges to the latest state. When `MQTT_DISCOVERY_PREFIX` is set, `OnConnect` also publishes a retained **HA MQTT discovery** config (`<prefix>/event/<sanitized-client-id>/doorbell/config`, an `event` entity with `device_class: doorbell` whose `value_template` supplies the event JSON since the press payload is empty). On shutdown `ProcessQueue` **drains**: already-queued fresh events get a best-effort flush bounded by `drainTimeout` = 2s, never waiting for a reconnect; and while holding an event, `deliver` wakes on an `OnConnect` signal instead of only the 1s poll.
...
- `main_test.go` is an end-to-end smoke test: it boots the real `main()` against an embedded `mochi-mqtt` broker (test-only dependency) and a fake camera, asserting press delivery, availability, discovery, `/health`, and SIGTERM shutdown. It is the only test allowed to call `main()` (the `-healthcheck` flag registration is once-only), and it trims `os.Args` first so `main`'s `flag.Parse` doesn't see `go test`'s flags.

## Release flow

`release-agent.yml` runs `scripts/release-agent.sh` weekly (Mondays 09:00 UTC) or on dispatch. Proposals are **gated on shipped paths** (`main.go`, `pkg/`, `go.mod`, `go.sum`, `Dockerfile`, `.dockerignore`): CI/docs/devcontainer-only churn never proposes, and bump detection + the changelog read only commits touching those paths, so a CI-only `feat` cannot inflate the bump. Chore-only shipped changes **accumulate for 21 days** (`CHORE_MAX_AGE_DAYS`) before proposing; `feat`/`fix`/breaking propose immediately, and a manual dispatch sets `FORCE_PROPOSAL=true` to skip the wait. The bump follows Conventional Commits (a `!` marker after the type/scope or a `BREAKING CHANGE` footer → major, `feat` → minor, otherwise patch — detected from `--format=%s`/`%B`, since `--oneline`'s hash prefix defeats subject anchors). The issue labeled `release-agent` is opened, or updated **only when the proposal content actually changed** — an ignored proposal does not re-notify weekly. `scripts/release-agent-test.sh` (run in CI) exercises every gate branch in scratch repos. The proposal records the HEAD SHA it was generated from (`**Commit:**` line). Adding the `release-approved` label triggers `release-publish.yml`, which tags **that recorded commit** (not main's tip — commits landing after review don't ship), cuts the GitHub release, calls `docker-publish.yml` to build, push, and cosign-sign the image, then verifies the version manifest actually exists on ghcr before closing the issue; any failure comments on the issue with the run link. Issue title/body reach the workflow shell via `env:` — never template `${{ github.event.issue.* }}` into a `run:` script; the body carries commit text.

Images carry SLSA provenance (`mode=max`) and SBOM attestations on the index, and are tagged `vX.Y.Z` + `latest` (linux/amd64 + linux/arm64) on every publish path: the approval flow above (`workflow_call` — the tag it creates comes from `GITHUB_TOKEN`, which GitHub never lets trigger other workflows, hence the explicit call), a hand-pushed `v*.*.*` tag (image only — no GitHub release), or Actions → Docker → Run workflow with a version to re-publish an existing tag (the recovery knob for a release whose docker leg failed, like v0.1.2/v0.1.3 — and v0.2.0's cache-service failure; note this path does not close the proposal issue, only the label-triggered flow does).
