...
Required (`envconfig`, fatal if missing): `MQTT_BROKER_URL`, `MQTT_CLIENT_ID`, `MQTT_USERNAME`, `MQTT_PASSWORD`, `HOSTNAME_OR_IP`, `DAHUA_USERNAME`, `DAHUA_PASSWORD`. Optional: `MQTT_TOPIC` (default `doorbell/pressed`), `MQTT_AVAILABILITY_TOPIC` (default `doorbell/availability`), `HEALTH_PORT` (default `8080`), `APP_ENV=development` for human-readable console logging instead of JSON.
